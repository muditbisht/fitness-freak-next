...
import VoteDiv from '@/components/VoteCountDiv';
...
export default function Answer({ answer }: AnswerProps) {
      ...
      <AnswerHeadlineDiv>
        <div className="user-info-div">
          ...
          <div className='user-name-div'>
            {answer.user ?
                <Link href={`/profile/${answer.user._id}`} className='user-username'>
                  {answer.user.username}
                  {answer.user.is_verified && <CheckCircleIcon style={{fontSize: 20}} color="primary" />}
                </Link>
  ...
  function deleteAnswer() {
    showPopup({
      content: { content: 'Do you want to delete this answer', title: 'Delete Answer' },
      agreeBtn: "Cancel",
      disagreeBtn: "Delete",
      onAgree: async () => { },
      onDisagree: del
