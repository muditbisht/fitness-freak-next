...
import FollowBtn from '@/components/FollowUnfollowBtn';
...
export default function SuggestionBox(){
    ...
    if (!loaded) {
        return <Div><StyledSpinner /></Div>;
    }

    if (!suggestions) {
        return <Div> Error, please load again </Div>;
    }
    ...
    return <Div>
                ...
                <Content>
                    ...
                    <Profile key={`user-profile-${user._id}`}>
                        <Avatar src={user.profile_image}/>
                        <div className="name">
                            <div className="main-name">{user.first_name} {user.last_name}
                                {user.is_verified && <CheckCircleIcon style={{fontSize: 20}} color="primary" />}
                            </div>
                            <Link className="username" href={`/profile/${user._id}`}>@{user.username}</Link>
                        </div>
